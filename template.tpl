___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Partnerize Custom Tracking Tag",
  "brand": {
    "id": "github.com_webmasterjunkie",
    "displayName": "Partnerize",
    "thumbnail": "data:image/png;base64,\u003d"
  },
  "description": "If you do not have a \"standard\" or \"enhanced\" structured dataLayer, or want to customize item data, you can use this template to pass your custom data.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "CHECKBOX",
    "name": "customTrackingEnabled",
    "checkboxText": "Custom Tracking Domain",
    "simpleValueType": true,
    "alwaysInSummary": true,
    "help": "Enable this if you are using a custom tracking domain.",
    "subParams": [
      {
        "type": "TEXT",
        "name": "customTrackingDomain",
        "displayName": "Custom Tracking URL",
        "simpleValueType": true,
        "enablingConditions": [
          {
            "paramName": "customTrackingEnabled",
            "paramValue": true,
            "type": "EQUALS"
          }
        ],
        "defaultValue": "https://prf.hn/",
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          },
          {
            "type": "REGEX",
            "args": [
              "^https?:\\/\\/(?:[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?\\.)+[a-zA-Z]{2,63}(?:\\/.*)?$"
            ],
            "errorMessage": "You must provide a fully qualified domain."
          }
        ]
      }
    ],
    "displayName": "Do not enable this setting without consulting Partnerize support first, as this will break tracking if incorrectly configured."
  },
  {
    "type": "GROUP",
    "name": "conversionDataGroup",
    "displayName": "Conversion Level Parameters",
    "groupStyle": "ZIPPY_OPEN",
    "subParams": [
      {
        "type": "LABEL",
        "name": "conversionLabel",
        "displayName": "Provide the parameters you will be sending to Partnerize, along with the values that relate to them. A mapping of these can be provided to you."
      },
      {
        "type": "SIMPLE_TABLE",
        "name": "conversionDataTable",
        "displayName": "",
        "simpleTableColumns": [
          {
            "defaultValue": "",
            "displayName": "Parameter",
            "name": "conversionParameter",
            "type": "TEXT",
            "isUnique": true,
            "valueValidators": [],
            "valueHint": ""
          },
          {
            "defaultValue": "",
            "displayName": "Value",
            "name": "conversionValue",
            "type": "TEXT",
            "valueHint": ""
          }
        ],
        "newRowButtonText": "Add Parameter"
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "conversionItemGroup",
    "displayName": "Conversion Item Parameters",
    "groupStyle": "ZIPPY_CLOSED",
    "subParams": [
      {
        "type": "LABEL",
        "name": "itemLabel",
        "displayName": "Provide the variable the contains all items being purchased. This should be an \"array\" of data. If left blank, no \"item\" data will be sent to Partnerize."
      },
      {
        "type": "TEXT",
        "name": "conversionItemVariable",
        "displayName": "Conversion Item Array",
        "simpleValueType": true
      },
      {
        "type": "SIMPLE_TABLE",
        "name": "conversionItemTable",
        "displayName": "Conversion Item Mapping",
        "simpleTableColumns": [
          {
            "defaultValue": "",
            "displayName": "Parameter",
            "name": "itemParameter",
            "type": "SELECT",
            "selectItems": [
              {
                "value": "category",
                "displayValue": "Category"
              },
              {
                "value": "sku",
                "displayValue": "SKU"
              },
              {
                "value": "quantity",
                "displayValue": "Quantity"
              },
              {
                "value": "value",
                "displayValue": "Item Value"
              }
            ]
          },
          {
            "defaultValue": "",
            "displayName": "Value",
            "name": "itemValue",
            "type": "TEXT",
            "valueHint": "Parameter Name"
          }
        ],
        "enablingConditions": [
          {
            "paramName": "conversionItemVariable",
            "paramValue": "",
            "type": "PRESENT"
          }
        ],
        "newRowButtonText": "Add Parameter"
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

const encodeUriComponent = require('encodeUriComponent');
const log = require('logToConsole');
const sendPixel = require('sendPixel');
const getCookieValues = require('getCookieValues');
const localStorage = require('localStorage');

const localStorageID = 'partnerizeClickReference';
const trackingDomain = setTrackingDomain(data);

let partnerize = trackingDomain + 'conversion/tmethod:1/tplatform:4/tdetail:10/';

if (data.conversionDataTable.length > 0){
  data.conversionDataTable.forEach(addConversionParameter);
}

if (typeof data.conversionItemVariable !== 'undefined' && data.conversionItemVariable !== null){
  if (data.conversionItemVariable !== ""){
    parseConversionItemArray(data.conversionItemVariable);
  }
}

handleClickrefVariable();

log(partnerize);

// Call data.gtmOnSuccess when the tag is finished.
sendPixel(partnerize, data.gtmOnSuccess, data.gtmOnFailure);

// Enable logging for debugging purposes
log(partnerize);
log(data);

function setTrackingDomain(data) {
  let domain = "https://prf.hn/";

  if (data.customTrackingEnabled) {
    domain = data.customTrackingDomain;
  }

  if (domain.slice(-1) === "/") {
    return domain;
  } else {
    return domain + "/";
  }
}

function handleClickrefVariable() {
  let clickref = localStorage.getItem(localStorageID);

  if (clickref == '' || clickref == null){
    var cookie = getCookieValues(localStorageID);
    if (cookie[0]){
      clickref = cookie[0];
    }
  }

  if (clickref != '' && clickref != null){
    if (partnerize.indexOf('clickref:/') !== -1){
      partnerize.replace('clickref:/', '/clickref:' + clickref);
    }

    if (partnerize.indexOf('clickref') === -1){
      partnerize = partnerize + '/clickref:' + clickref;
    }
  }
}

function parseConversionItemArray(products){
  // Loop through the items in the array
  for (const index in products){
    let item = products[index];
    let matches = [];
    partnerize = partnerize + '[';

    if (data.conversionItemTable.length > 0){

      // Loop through the mapping table
      for (const mapping in data.conversionItemTable){
        if (item.hasOwnProperty(data.conversionItemTable[mapping].itemValue)){
          if (typeof item[data.conversionItemTable[mapping].itemValue] !== 'undefined' && item[data.conversionItemTable[mapping].itemValue] !== null){
            partnerize = partnerize + data.conversionItemTable[mapping].itemParameter + ':' + encodeUriComponent(item[data.conversionItemTable[mapping].itemValue].toString()) + '/';
          } else {
            partnerize = partnerize + data.conversionItemTable[mapping].itemParameter + ':/';
          }
          matches.push(data.conversionItemTable[mapping].itemValue);
        }
      }
    }

    for (const key in item){
      if (matches.indexOf(key) === -1){
        partnerize = partnerize + formatParameter(key) + ':' + encodeUriComponent(item[key].toString()) + '/';
      }
    }

    partnerize = partnerize + ']';
  }
}

function addConversionParameter(item, index){
  partnerize = partnerize + formatParameter(item.conversionParameter) + ':' + encodeUriComponent(item.conversionValue.toString()) + '/';
}

function formatParameter(customParameter){
  var string = customParameter.replace(' ', '_');
  var regex = "/[^A-Za-z0-9_]/";

  return string.replace(regex, '', string).toLowerCase();
}


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "send_pixel",
        "versionId": "1"
      },
      "param": [
        {
          "key": "allowedUrls",
          "value": {
            "type": 1,
            "string": "any"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "get_cookies",
        "versionId": "1"
      },
      "param": [
        {
          "key": "cookieAccess",
          "value": {
            "type": 1,
            "string": "specific"
          }
        },
        {
          "key": "cookieNames",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "partnerizeClickReference"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_local_storage",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "partnerizeClickReference"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 1/28/2022, 1:20:14 PM
